...
export const COMMANDS: CommandInfo[] = [
  // ── Core ──
  {
    ...
    aliases: [],
    category: 'core',
    summary: 'Start Ved daemon with live TUI dashboard',
    usage: 'ved start [--simple] [--port <port>] [--host <host>] [--token <token>]',
    flags: ['--simple', '--no-tui', '--port <port>', '--host <host>', '--token <token>', '--cors <origin>'],
    examples: ['ved', 'ved start', 'ved start --port 3000', 'ved start --simple'],
  },
  {
    name: 'init',
    aliases: [],
    category: 'core',
    summary: 'Interactive setup wizard — choose provider, set API key, configure trust',
    usage: 'ved init [--force] [--yes]',
    examples: ['ved init', 'ved init --force', 'ved init -y'],
  },
  {
    name: 'version',
    aliases: ['--version', '-v'],
  ...
  },
  {
    ...
    category: 'data',
    summary: 'Sync vault to/from remote endpoints (git, s3, rsync, local)',
    usage: 'ved sync <subcommand> [options]',
    subcommands: ['list', 'add', 'remove', 'push', 'pull', 'status', 'history'],
    flags: ['--force', '--limit <n>', '--failed-only', '--show-auth', '--auth <data>'],
    examples: [
      'ved sync',
      'ved sync add origin git https://github.com/user/vault.git',
      'ved sync add backup local /backups/vault',
      'ved sync add s3-backup s3 s3://my-bucket/vault',
      'ved sync push origin',
      'ved sync pull origin --force',
      'ved sync status',
      'ved sync history --limit 20',
      'ved sync history --failed-only',
      'ved sync remove origin',
    ],
  },
  {
    name: 'migrate',
    ...
    category: 'data',
    summary: 'Import data from external sources (markdown, JSON, CSV, Obsidian, JSONL)',
    usage: 'ved migrate <subcommand> [options]',
    subcommands: ['status', 'markdown', 'json', 'obsidian', 'csv', 'jsonl', 'undo', 'history', 'validate'],
    flags: ['--dry-run', '--force', '--tag=<tag>', '--folder=<folder>', '-r', '--name-col=<col>'],
    examples: [
      'ved migrate status',
      'ved migrate markdown ~/notes/ -r --tag=imported',
      'ved migrate json chatgpt-export.json',
      'ved migrate obsidian ~/my-vault --tag=old-vault',
      'ved migrate csv contacts.csv --name-col=full_name',
      'ved migrate jsonl conversations.jsonl',
      'ved migrate validate json export.json',
      'ved migrate undo abc12345',
      'ved migrate history',
    ],
  },
  {
  ...
  // ── Configuration ──
  {
    ...
    aliases: [],
    category: 'config',
    summary: 'Validate, show, edit, or locate Ved configuration',
    usage: 'ved config <validate|show|path|edit [local]>',
    subcommands: ['validate', 'show', 'path', 'edit'],
    examples: ['ved config validate', 'ved config show', 'ved config path', 'ved config edit', 'ved config edit local'],
  },
  {
    name: 'env',
